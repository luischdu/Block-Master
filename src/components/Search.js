...
import { movieListPopular, search, watchListLater } from "../action/movie";
...
const Search = () => {
  ...
  const list = useSelector((state) => state.movie.search);
  ...
  const [query, setquery] = useState("casa");

  useEffect(() => {
    axios
      .get(
        `https://api.themoviedb.org/3/search/movie?api_key=ed4ef444cf09035de37c391527885e55&language=en&query=${query}&page=${page}&include_adult=false`
      )
      .then(async ({ data }) => {
        await dispatch(search([...list, ...data.results]));
