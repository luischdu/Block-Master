...
import { movieList } from "../action/movie";
...
const Home = () => {
  ...
  const list = useSelector((state) => state.movie.hmovie);
  ...
  useEffect(() => {
    axios
      .get(
        `https://api.themoviedb.org/3/movie/popular?api_key=ed4ef444cf09035de37c391527885e55&language=es&page=${page}`
      )
      .then(async ({ data }) => {
        await dispatch(movieList([...list, ...data.results]));
        console.log(data);
        setIsLoading(false);
      })
      .catch((e) => {
